(function () {
  ...
  var common = {
    ...
    'الشركة': 'Company', 'شروط التعامل القياسية (PDF)': 'Standard Trading Conditions (PDF)', 'استعرض دليل الموانئ': 'Explore the port directory',
    'سنة التأسيس في السعودية': 'Founded in Saudi Arabia', 'ميناءً في الدليل': 'Ports in the directory', 'مكاتب في المملكة': 'Offices across the Kingdom', 'خدمات معتمدة': 'Integrated services',
  ...
  function english() {
    ...
    addToggle(); var t=document.getElementById('lang-toggle'); if(t){t.textContent='Arabic';t.setAttribute('aria-label','Switch to Arabic');t.setAttribute('aria-pressed','true');t.addEventListener('click',function(){localStorage.setItem('sharaf-language','ar');location.reload();});}
    ...
    if(key==='network'){setText('.prose > p','Sharaf Shipping operates one of the Kingdom’s most connected regional networks. Clients benefit from broad coverage of the main Saudi markets through offices grouped into Eastern and Western regions.');}
    ...
    var about=document.querySelector('.foot .about');if(about)about.textContent='Sharaf Shipping Agency Co. Ltd — Kingdom of Saudi Arabia. A marine agency, freight and logistics partner established in 2007 as part of the Sharaf Group, with nine offices covering the Kingdom’s Eastern and Western ports.';
    document.querySelectorAll('.foot h4').forEach(function(h){if(h.textContent.trim()==='الشركة')h.textContent='Company';});
    document.querySelectorAll('.foot a').forEach(function(a){if(a.textContent.indexOf('شروط التعامل القياسية')>=0)a.textContent=a.textContent.replace('شروط التعامل القياسية','Standard Trading Conditions').replace('القياسية','');});
    var base=document.querySelector('.foot-base');if(base){var spans=base.querySelectorAll(':scope > span');if(spans[0])spans[0].textContent='© 2017 Sharaf Shipping Agency Co. Ltd, Kingdom of Saudi Arabia. All rights reserved.';if(spans[1])spans[1].innerHTML='Working hours: Sunday – Thursday <span class="mono">08:00 – 17:00</span>';}
